...
#i =1
def show_pic(img, file, bboxes=None):
    '''
    输入:
        img:图像array
    ...
    '''
    #global i
    print "{}_{}.jpg".format(file[:-4],i)
    #cv2.imwrite("img/{}_{}.jpg".format(file[:-4],i),img)
    ...
    #cv2.namedWindow(img, 0)  # 1表示原图
    #cv2.moveWindow(img, 0, 0)
    #cv2.resizeWindow(img, 1200,800)  # 可视化的图片大小
    cv2.imwrite("{}_{}.jpg".format(file[:-4],i),img)
    '''
    if i<5:
        i+=1
    else:
...
class DataAugmentForObjectDetection():
    ...
    def _filp_pic_bboxes(self, img, bboxes):
        ...
        for box in bboxes:
            ...
            y_max = box[3]
            if horizon:
                flip_bboxes.append([w-x_max, y_min, w-x_min, y_max,box[-1]])
            else:
                flip_bboxes.append([x_min, h-y_max, x_max, h-y_min,box[-1]])
    ...
    for parent, _, files in os.walk(source_pic_root_path):
        for file in files:
            ...
            while cnt < need_aug_num:
                ...
                #show_pic(img, coords[:4])    # 原图

                auged_img, auged_bboxes = dataAug.dataAugment(img, coords)
                #print auged_bboxes
                generate_xml(file,auged_bboxes,auged_img.shape,"./xml/",i)
                cnt += 1
                cv2.imwrite("img/{}_{}.jpg".format(file[:-4],i),auged_img)
                print "{}_{}.jpg".format(file[:-4],i)
                if i<need_aug_num:
                    i+=1
                else:
                    i=1
                #cv2.imshow("test",auged_img)
                #cv2.waitKey()

                #show_pic(auged_img, file, auged_bboxes)  # 强化后的图
